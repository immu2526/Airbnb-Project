...
import { updateListing } from "../../store/listing-Slice";
...
const EditListing = () => {
  ...
  useEffect(() => {
    let listingDetails = data.find((val) => {
      return val._id === id;
    });
    if (listingDetails) {
      setForm({
      ...
      if (listingDetails.image?.url) {
        setImage({
          file: null,
          url: listingDetails.image.url,
          filename: listingDetails.image.filename,
        });
      }
    }
  }, []);
  ...
  return (
    <>
      ...
      <div className="max-w-lg mx-auto mt-10 p-6 bg-white rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-6">Edit Listing</h2>
